...
/*	self.hide = ():void =>{
	...
	function loadModule(mod: IModule | string): void {
		...
		function ldM(mod:string):boolean {
			...
			//	case "font":				getStyle("https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css"); setReady(mod); return true;
				...
				case "tree":				loadModule('helperTree');
			...
			/*	case "profileMe":			$('#'+mod).load( loadPath+'modules/profileMe-0.93.1.mod.html', function() {setReady(mod)} ); return true;
				...
				case 'toHtml': getScript(loadPath + 'modules/specif2html.js'); return true;
			...
			/*	case CONFIG.objectTable:  	loadModule( 'dataTable' );
				...
				case 'importAny': loadModule('zip');
								...
								getScript(loadPath + 'modules/'+mod+'.mod.js'); return true;
				...
				case 'ioReqif': getScript(loadPath + 'modules/ioReqif.mod.js'); return true;
	...
	function setReady( mod:string ):void {
		...
		if( self.registered.length === self.ready.length ) {
			...
			getStyle(loadPath + 'assets/stylesheets/SpecIF.default.css');  // must be loaded last as it defines overriding styles/classes
