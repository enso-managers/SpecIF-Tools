...
moduleManager.construct({
	...
	/*	^... not any more necessary as importAny filters all invalid file-types during selection ...
...
function xlsx2specif(buf: ArrayBuffer, prjN:string, chAt:string):SpecIF {
	"use strict";
	// requires sheetjs
	...
	class Worksheet {
		...
		constructor(wsN: string) {
			...
			this.hid = CONFIG.prefixH + simpleHash(prjN + wsN)	// the hierarchy ID of the folder carrying all resources of the selected sheet
		...
		if (ws && ws.isValid) {
			...
			switch (ws.name) {
				...
				default:
					...
					// else: all worksheets with other names will be processed by transformData()
			}
		}
	}
	...
	function transformData(ws: Worksheet): void {
		if (ws && ws.isValid) {
			...
			if (ws.name.startsWith("{") && ws.name.endsWith("}")) return;
...
//	console.debug('workbook',prjN,wb);
	...
	// Take the filename without extension and without [resource type] as project title:
	specifData.title = LIB.makeMultiLanguageValue(
		withoutBracketsAtEnd(fName.fileName())
	);
...
//	console.info('SpecIF created from '+prjN+' (Excel)');
//	console.debug('SpecIF',specifData);
	return specifData;

	function withoutBracketsAtEnd(str: string): string | undefined {
		// Remove resourceClass in {curly brackets}:
		//	let resL = /\s*(?:\(|\[)([a-zA-Z\d:_\-].+?)(?:\)|\])$/.exec( prjN );
